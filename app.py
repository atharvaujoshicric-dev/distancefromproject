...
def extract_area_logic(text):
    ...
    for i in range(1, len(f_segments), 2):
        ...
        if 0 < val < 5000 and not any(word in context_before for word in parking_keywords):
            f_vals.append(val)
    if f_vals:
...
st.title("🏠 Property Analysis with Project Metadata")
...
st.sidebar.markdown("---")
show_extra_info = st.sidebar.checkbox("Add Project Details (Amenities, Towers, etc.)")
...
if uploaded_file:
    ...
    if desc_col and cons_col and prop_col:
        with st.spinner('Calculating...'):
            ...
            # --- NEW ADDITIONS LOGIC ---
            if show_extra_info:
                # This part represents the logic to add extra columns. 
                # Since these values are external to the sales dump, we initialize them.
                # In a real scenario, you can map these from a master project list.
                summary['Amenities'] = "N/A"
                summary['Towers'] = "N/A"
                summary['Floors'] = "N/A"
                summary['Total Units'] = "N/A"
                summary['Possession'] = "N/A"
            ...
            st.download_button(label="📥 Download Excel Report", data=output.getvalue(), file_name="Property_Report.xlsx")
    else:
