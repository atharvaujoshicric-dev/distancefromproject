...
def apply_excel_formatting(df, writer, sheet_name):
    ...
    # Function to merge identical vertical cells
    def merge_cells_for_col(col_idx, group_by_col_indices):
        start_row = 2
        for i in range(2, len(df) + 2):
            # Check if values for all parent groups and current group are same
    ...
    merge_cells_for_col(1, [0])    # Merge Property Column
    merge_cells_for_col(2, [0, 1]) # Merge Configuration Column (nested within Property)

# --- STREAMLIT UI ---
st.set_page_config(page_title="Real Estate Summary Tool", layout="wide")
st.title("🏠 Advanced Property Summary Tool")
...
if uploaded_file:
    ...
    if desc_col and cons_col and prop_col:
        with st.spinner('Generating Formatted Report...'):
            ...
            # Grouping including unique Carpet Areas
            ...
            summary[['Min. APR', 'Max APR', 'Average of APR']] = summary[['Min. APR', 'Max APR', 'Average of APR']].round(3)
            ...
            st.download_button(label="📥 Download Merged Excel Report", data=output.getvalue(), file_name="Property_Summary_Merged.xlsx")
    else:
